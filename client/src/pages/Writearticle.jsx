...
const handlesubmit = async (e) => {
  ...
  try {
    ...
    const prompt = `write an article about ${input} in ${selected.text} words`;

    const { data } = await axios.post('http://localhost:3000/api/ai/generate-article',
      { prompt, length: selected.length },
      {
        headers: {
           ...
           <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
            {
              articlelength.map((item,index)=>(
                <span  onClick={()=> setSelected(item)}  key={index} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selected.text===item.text ? 'bg-blue-50 text-blue-700' : 'text-gray-500 border-gray-300'} `}>{item.text}</span>
              ))
            }
           </div>
           <br></br>
  ...
  <h1 className='text-xl font-semibold'>
    Generated Articles
       ...
       {!content ? (<div className='flex-1 flex justify-center items-center'>
            ...
            <p>Enter a topic and click "Generate Articles" to create a list of articles based on that topic.</p>
            </div>
